...
class Index extends Controller
{
    ...
    private function step5()
    {
        ...
        // 导入系统初始数据库结构
        // 导入SQL
        $sql_file = APP_PATH.'install/sql/madou.sql';
        ...
        return $this->success('系统安装成功，欢迎您使用苹果CMS建站，请勿用于违法违规的行为，否则后果自负。', $root_dir.'MDadmin.php');//安装成功提示，跳转到后台地址
    }
        ...
        private function checkFunc()
    {
        $items = [
			...
			['sg11', lang('install/support'), 'yes', ' 模块','sg_load'],
        ];
        ...
        foreach ($items as &$v) {
            if(('类'==$v[3] && !class_exists($v[0])) || (lang('install/model')==$v[3] && !extension_loaded($v[0])) || (lang('install/function')==$v[3] && !function_exists($v[0])) || (lang('install/config')==$v[3] && ini_get('always_populate_raw_post_data')!=-1) || ('sg11'==$v[0] && !function_exists($v[4]))) {
                $v[1] = '不支持';
                $v[2] = 'no';
                session('install_error', true);
            }
