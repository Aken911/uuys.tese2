...
class User extends Base
{
    public function __construct()
    {
		...
		$count = Db::table('mac_user')->where('user_pid',$user_id)->count('user_id');
    ...
    public function reg()
    {
        ...
        if (Request()->isPost()) {
			...
			//邀请赠送天数
			if (!empty($param['codes'])){
                $count = Db::table('mac_user')->where('user_pid',$ros['user_id'])->count('user_id');
                $invitenum1 = $GLOBALS['config']['site']['site_invitenum_0'];//邀请指定1
                $invitenum2 = $GLOBALS['config']['site']['site_invitenum_1'];//邀请指定2
                $invitenum3 = $GLOBALS['config']['site']['site_invitenum_2'];//邀请指定3
                $invitenum4 = $GLOBALS['config']['site']['site_invitenum_3'];//邀请指定4
				
                $invitetime1 = $GLOBALS['config']['site']['site_invitetime_0'];//奖励天数1
                $invitetime2 = $GLOBALS['config']['site']['site_invitetime_1'];//奖励天数2
                $invitetime3 = $GLOBALS['config']['site']['site_invitetime_2'];//奖励天数3
                $invitetime4 = $GLOBALS['config']['site']['site_invitetime_3'];//奖励天数4

                if ($count >= $invitenum1 && $count < $invitenum2){
                    $time = time()+(86400*$invitetime1); 
					$data=[
						'user_end_time'=>$time,
						'group_id'=>3,
					];
                }elseif ($count >= $invitenum2 && $count < $invitenum3){
					$time = time()+(86400*$invitetime2); 
					$data=[
						'user_end_time'=>$time,
						'group_id'=>3,
					];
                }elseif ($count >=$invitenum3 && $count < $invitenum4){
					$time = time()+(86400*$invitetime3); 
					$data=[
						'user_end_time'=>$time,
						'group_id'=>3,
					];
                }elseif ($count >=$invitenum4){
					$time = time()+(86400*$invitetime4); 
					$data=[
						'user_end_time'=>$time,
						'group_id'=>3,
					];
                ...
                //更改值
				Db::table('mac_user')->where('user_name',$ros['user_name'])->setField($data);
				
            }

			//注册赠送VIP天数
            if (!empty($GLOBALS['config']['site']['site_reg_vip'])){
                $time = time()+(86400*$GLOBALS['config']['site']['site_reg_vip']);
                $data=[
                    'user_end_time'=>$time,
                    'group_id'=>3,
                    'user_num'=>$GLOBALS['config']['site']['site_see_2'],
                ];
                Db::table('mac_user')->where('user_name',$param['user_name'])->setField($data);
            }
	...
	public function picture()
    {
        ...
        $this->assign('param',$param);
        $this->assign('list', $res['list']);
        $pages = mac_page_param($res['total'], $param['limit'], $param['page'], url('user/picture', ['page' => 'PAGELINK']));
        $this->assign('__PAGING__', $pages);
        return $this->fetch('user/picture');
    }
	...
	public function novel()
    {
        ...
        $this->assign('param',$param);
        $this->assign('list', $res['list']);
        $pages = mac_page_param($res['total'], $param['limit'], $param['page'], url('user/novel', ['page' => 'PAGELINK']));
        $this->assign('__PAGING__', $pages);
        return $this->fetch('user/novel');
    }
